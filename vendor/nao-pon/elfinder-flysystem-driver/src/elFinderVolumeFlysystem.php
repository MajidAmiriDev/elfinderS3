...
class elFinderVolumeFlysystem extends elFinderVolumeDriver {
    ...
    protected function _chmod($path, $mode) {
        // Flysystem doesn't directly support chmod, so you can return false
        return false;
    }
     ...
     * @param  string   $mode       action how to manipulate image
     * @param  string   $bg         background color (for some modes)
     * @param  int      $degree     rotation degree
     * @param  int|null $jpgQuality JPEG quality (optional)
     * @return array|false
     * @author ...
     **/
    public function resize($hash, $width, $height, $x, $y, $mode = 'resize', $bg = '', $degree = 0, $jpgQuality = null) {
        ...
        // Create an image instance using the image manager
        if (!$image = $this->imageManager->make($this->_getContents($path))) {
            return false;
        }

        // Handle different modes for resizing or manipulating the image
        switch ($mode) {
            case 'propresize':
        ...
        // Adjust the JPEG quality if specified
        if ($jpgQuality !== null) {
